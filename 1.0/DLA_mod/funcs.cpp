  ...
  for (int i=0;i<np;i++){
   ...
   for (int p=0;p<np;p++)
    ...
    for (int i=0;i<np;i++){
        ...
        f[i][1]=c_be[i];
        f[i][2]=c_ga[i];
        //cout << "["<<c_al[i]<<","<<c_be[i]<<","<<c_ga[i]<<"],"<<endl;
    }
}
...
double get_overlaps1(vector<vector<double>>cluster,double overlaps[] ,double rod[][3],bool &overlap,bool &success,int ar){
...
double get_overlaps(vector<vector<double>>cluster,double overlaps[] ,double rod[][3],bool &overlap,bool &success,int ar){
    ...
    overlap = false;
    success = false;
    for (int j=0;j<cluster.size();j++){// j in range(len(cluster[:,0])):
        for (int p=0;p<3;p++){
...
//        v1[3] = {cluster[j][0]-rod[0][0],cluster[j][1]-rod[0][1],cluster[j][2]-rod[0][2]};
  //      v2[3] = {cluster[j][0]-rod[ar-1][0],cluster[j][1]-rod[ar-1][1],cluster[j][2]-rod[ar-1][2]};
...
double neighbour_list(vector <vector<double>> &nl, double r[3], vector <vector<double>> cluster, double limr){
    ...
    int i=0;
    //#if np.dot(delta,r)<0:
    for (int j=0;j<cluster.size();j++){//for j in range(len(cluster)):
        //#d = ((cluster[j,:]-r)[0]**2+(cluster[j,:]-r)[1]**2+(cluster[j,:]-r)[2]**2)**.5
